        ...
        else 
        {
            ...
            if ($form->isSubmitted() && $form->isValid()) 
            {
                ...
                foreach ($this->panierService->getDetailsPanierProduits() as $panierProduit) 
                {
                    ...
                    if ($panierProduit->produit->isBilletDeSession() == 1) 
                    {
                        $billetDeSession = new BilletDeSession;

                        $billetDeSession->setDateBilletDeSessionAt(new \DateTime('now'))
                    ...
                    if ($panierProduit->produit->isExamen() == 1) 
                    {
                        ...
                        $listeExamenAFaire->setExamen($panierProduit->produit)
                                            ->setResultatExamen($resultatExamen);
                        
                        $this->em->persist($listeExamenAFaire);
                    }
