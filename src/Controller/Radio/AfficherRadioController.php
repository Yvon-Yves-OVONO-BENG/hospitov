...
class AfficherRadioController extends AbstractController
{
    ...
    #[Route('/afficher-radio/{a<[0-1]{1}>}/{m<[0-1]{1}>}/{s<[0-1]{1}>}', name: 'afficher_radio')]
    public function afficherRadio(Request $request, int $a = 0, int $m = 0, int $s = 0): Response
    {
        ...
        #je récupère toutes les radios
        $radios = $this->radioRepository->findBy([
            'radio' => 1
        ]);
        
        #j'envoie mon tableau des radios à mon rendu twig pour affichage
        return $this->render('radio/afficherRadio.html.twig', [
            'licence' => 1,
            'radios' => $radios,
